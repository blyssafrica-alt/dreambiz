-- FIXED: Database trigger to automatically create user profile ONLY when a NEW user signs up
-- This bypasses RLS issues by running with elevated privileges
-- Run this in Supabase SQL Editor with "No limit" selected

-- IMPORTANT: This trigger ONLY fires on NEW user creation (AFTER INSERT)
-- It will NOT create profiles for existing users unless they sign up again
...
-- Create trigger that fires ONLY when a NEW user is created in auth.users
-- This will NOT fire for existing users
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW
  EXECUTE FUNCTION public.handle_new_user();

-- Note: The sync_existing_users() and sync_user_profile() functions below
-- are ONLY for manual syncing of existing users. They do NOT run automatically.
-- They should only be called manually when needed.

-- Function to sync existing users (ONLY call this manually when needed)
...
-- RPC function to sync a single user profile (ONLY call this manually when needed)
...
-- IMPORTANT NOTES:
-- 1. The trigger ONLY fires on NEW user signups (AFTER INSERT)
-- 2. It will NOT create profiles for existing auth.users
-- 3. If you need to sync existing users, manually call: SELECT public.sync_existing_users();
-- 4. If you need to sync a specific user, manually call: SELECT public.sync_user_profile('user-id-here'::UUID);
