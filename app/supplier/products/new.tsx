...
import { ArrowLeft, ChevronLeft, ChevronRight, DollarSign, Lightbulb, Package, Plus, Send, Sparkles, Trash2, Upload, X } from 'lucide-react-native';
...
export default function SupplierProductNewScreen() {
  ...
  const [autosaveStatus, setAutosaveStatus] = useState<'idle' | 'saving' | 'saved' | null>(null);
  ...
  const suggestedSubcategory = (() => {
    ...
    if (!n || subcategories.length === 0) return null;
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (profile) {
        ...
        const [canRes, subRes] = await Promise.all([
  ...
  const buildPayload = useCallback(
    (targetStatus?: 'draft' | 'pending' | 'published') => {
      const slug = name.trim().toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') || 'product-' + Date.now();
      ...
      if (targetStatus !== undefined) base.status = targetStatus;
      return base;
    },
    [name, shortDescription, description, price, currency, minOrderQty, availabilityStatus, sku, unitType, leadTimeDays, priceType, subcategoryId, tierPrices, specifications, imageUrls],
  );

  const runAutosave = useCallback(async () => {
    if (!profileId || !name.trim() || saving) return;
    setAutosaveStatus('saving');
    try {
      const payload = buildPayload('draft');
      if (draftId) {
        const { error } = await supabase.from('supplier_marketplace_products').update(payload).eq('id', draftId).eq('supplier_profile_id', profileId);
        if (error) throw error;
      } else {
        const slug = (payload.slug as string) + '-' + Date.now();
        const { data, error } = await supabase
          .from('supplier_marketplace_products')
          .insert({ ...payload, slug, supplier_profile_id: profileId })
          .select('id')
          .single();
        if (error) throw error;
  ...
  }, [profileId, name, saving, draftId, buildPayload]);
  ...
  }, [profileId, name, shortDescription, description, price, currency, minOrderQty, availabilityStatus, sku, unitType, leadTimeDays, priceType, subcategoryId, JSON.stringify(tierPrices), JSON.stringify(specifications), JSON.stringify(imageUrls), saving, runAutosave]);
  ...
  const save = async (targetStatus: 'draft' | 'pending' | 'published') => {
    ...
    try {
      ...
      if (draftId) {
        ...
        const { error } = await supabase.from('supplier_marketplace_products').insert({ ...payload, supplier_profile_id: profileId }).select('id').single();
        if (error) throw error;
      }
      const msg = targetStatus === 'draft' ? 'Product saved as draft.' : targetStatus === 'pending' ? 'Product submitted for admin review.' : 'Product published on your store.';
      RNAlert.alert('Saved', msg, [
        { text: 'OK', onPress: () => router.replace({ pathname: '/supplier/products', params: {} }) },
      ]);
    } catch (e: any) {
        ...
        <View style={[styles.card, { backgroundColor: theme.background.card }]}>
          {step === 'basics' && (
            <>
              ...
              <TextInput style={[styles.input, { backgroundColor: theme.background.secondary, color: theme.text.primary }]} placeholder="e.g. Industrial Grade Steel Rods 10mm" placeholderTextColor={theme.text.tertiary} value={name} onChangeText={setName} />
          ...
          {step === 'pricing' && (
            <>
              ...
              <Text style={[styles.helperText, { color: theme.text.tertiary }]}>Price breaks by quantity, e.g. 10+ at $9, 100+ at $8</Text>
              {tierPrices.map((t, i) => (
          ...
          {step === 'details' && (
            <>
              ...
              <TextInput style={[styles.input, { backgroundColor: theme.background.secondary, color: theme.text.primary }]} placeholder="unit, kg, box, etc." placeholderTextColor={theme.text.tertiary} value={unitType} onChangeText={setUnitType} />
          ...
          {step === 'media' && (
            <>
              ...
              <Text style={[styles.hintText, { color: theme.text.tertiary }]}>Products with 2+ photos get 40% more inquiries</Text>
          ...
          {step === 'submit' && (
            <View style={styles.reviewSection}>
              ...
              <Text style={[styles.progressLabel, { color: theme.text.tertiary }]}>{completionScore}% complete {completionScore < 80 ? '– fill more fields to improve visibility' : ''}</Text>
              ...
              <Text style={[styles.reviewRow, { color: theme.text.secondary }]}>Availability: {availabilityStatus}</Text>
        ...
        <View style={styles.footerRow}>
          ...
          {stepIndex < STEPS.length - 1 ? (
            ...
            <View style={styles.submitButtons}>
              <TouchableOpacity style={[styles.submitBtn, { backgroundColor: theme.background.secondary }]} onPress={() => save('draft')} disabled={saving}>
                {saving ? <ActivityIndicator size="small" color={theme.text.primary} /> : <Text style={[styles.submitBtnText, { color: theme.text.primary }]}>Save as draft</Text>}
              </TouchableOpacity>
              <TouchableOpacity style={[styles.submitBtn, { backgroundColor: '#3B82F6' }, !canSubmit && { opacity: 0.6 }]} onPress={() => (canSubmit ? save('pending') : RNAlert.alert('Complete required fields', tips[0] || 'Product name is required'))} disabled={saving}>
              ...
              {canPublish && (
                <TouchableOpacity style={[styles.submitBtn, { backgroundColor: theme.accent.primary }, !canSubmit && { opacity: 0.6 }]} onPress={() => (canSubmit ? save('published') : RNAlert.alert('Complete required fields', tips[0] || 'Product name is required'))} disabled={saving}>
                  {saving ? <ActivityIndicator size="small" color="#FFF" /> : <><DollarSign size={18} color="#FFF" /><Text style={[styles.submitBtnText, { color: '#FFF' }]}>Publish now</Text></>}
                </TouchableOpacity>
              )}
            </View>
          )}
