...
export default function AdminMailingTemplateDetailScreen() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const { data, error } = await supabase
        ...
        .select('id, name, html, variables, created_at')
        .eq('id', id)
        .single();
      if (error || !data) {
        setTemplate(null);
        ...
        setName(t.name);
        setHtml(t.html);
        setSelection({ start: t.html.length, end: t.html.length });
      }
      setLoading(false);
    };
    load();
  ...
  const save = async () => {
    if (!id || !name.trim()) {
      RNAlert.alert('Error', 'Name is required.');
      return;
    }
    setSaving(true);
    const { error } = await supabase
      .from('email_templates')
      .update({ name: name.trim(), html: html })
      .eq('id', id);
    setSaving(false);
    if (error) {
      RNAlert.alert('Error', error.message);
    } else {
      RNAlert.alert('Saved', 'Template updated.');
      setTemplate((prev) => prev ? { ...prev, name: name.trim(), html } : null);
    }
  };
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: theme.background.secondary }]} edges={['top']}>
        <View style={styles.centered}>
          ...
          <Text style={[styles.loadingText, { color: theme.text.tertiary }]}>Loading template…</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  if (!template) {
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: theme.background.secondary }]} edges={['top']}>
        ...
        />
        <View style={styles.centered}>
          <Text style={[styles.notFound, { color: theme.text.secondary }]}>Template not found or you don't have access.</Text>
          <TouchableOpacity style={[styles.btn, { backgroundColor: theme.accent.primary, marginTop: 16 }]} onPress={() => router.back()}>
            <Text style={[styles.btnText, { color: '#FFF' }]}>Back to templates</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background.secondary }]} edges={['top']}>
      <PageHeader
        title={name || 'Template'}
        subtitle="View and edit template"
        showLogo={false}
        icon={FileText}
        iconGradient={['#0EA5E9', '#0284C7']}
        leftAction={
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        <View style={[styles.section, { backgroundColor: theme.background.card }]}>
          <LinearGradient colors={['#0EA5E918', '#0284C708']} style={styles.sectionHeader}>
            ...
            <Text style={[styles.sectionSubtitle, { color: theme.text.tertiary }]}>Edit name and HTML content</Text>
          </LinearGradient>
          ...
          />
          <Text style={[styles.variableHint, { color: theme.text.tertiary }]}>Tap a variable to insert at cursor:</Text>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}
          >
            {saving ? <ActivityIndicator size="small" color="#FFF" /> : <Text style={[styles.btnText, { color: '#FFF' }]}>Save changes</Text>}
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
