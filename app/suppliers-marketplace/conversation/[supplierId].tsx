...
import { ArrowLeft, MessageSquare, Send, Image as ImageIcon, Paperclip } from 'lucide-react-native';
...
export default function SupplierConversationScreen() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      setSupplierName(profile.business_name || 'Supplier');
      setSupplierUserId((profile as any).user_id ?? null);
      ...
      if (existing) {
        ...
        const { data: created, error } = await supabase
          .from('supplier_conversations')
          .insert({ supplier_profile_id: supplierId, user_id: user.id })
          .select('id')
          .single();
        if (error || !created) {
          setLoading(false);
  ...
  const pickDocument = async () => {
    ...
    try {
      ...
      const url = await uploadBase64ToStorage(supabase, { bucket: BUCKET_MESSAGE_ATTACHMENTS, filePath: path, base64, contentType, upsert: false });
      setAttachmentUrls((u) => [...u, url]);
      setAttachmentNames((n) => [...n, name]);
    } catch (e: any) {
  ...
  if (!canAccess) {
    return (
      <View style={[styles.centered, { backgroundColor: theme.background.secondary }]}>
        <Text style={[styles.emptyTitle, { color: theme.text.primary }]}>Access not available</Text>
        <Text style={[styles.emptyBody, { color: theme.text.secondary }]}>Supplier messaging is not available for your plan.</Text>
      </View>
    );
  }
  if (!user?.id) {
    return (
      <View style={[styles.centered, { backgroundColor: theme.background.secondary }]}>
        <Text style={[styles.emptyTitle, { color: theme.text.primary }]}>Sign in to message</Text>
        <Text style={[styles.emptyBody, { color: theme.text.secondary }]}>Sign in to message this supplier.</Text>
  ...
  if (!conversationId) {
    return (
      <View style={[styles.centered, { backgroundColor: theme.background.secondary }]}>
        ...
        <Text style={[styles.emptyBody, { color: theme.text.secondary }]}>Could not start conversation. Please try again.</Text>
        <TouchableOpacity style={[styles.primaryBtn, { backgroundColor: theme.accent.primary }]} onPress={() => router.back()} activeOpacity={0.85}>
          <Text style={styles.primaryBtnText}>Go back</Text>
        </TouchableOpacity>
      </View>
