...
export async function getBookBySlug(slug: string): Promise<Book | null> {
  try {
    ...
    return {
      ...
      enabledFeatures: Array.isArray(data.enabled_features) ? data.enabled_features : (data.enabled_features || []),
      author: data.author,
      isbn: data.isbn,
...
export async function getAllPublishedBooks(): Promise<Book[]> {
  try {
    ...
    return data.map((row: any) => ({
      ...
      enabledFeatures: Array.isArray(row.enabled_features) ? row.enabled_features : (row.enabled_features || []),
      author: row.author,
      isbn: row.isbn,
